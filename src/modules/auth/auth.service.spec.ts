...
describe('authService', () => {
  ...
  it('should logout', () => {
    ...
    mockTokenService.deleteRefreshToken.mockImplementation(() =>
      of(loggedInUser),
    );
  ...
  it('should logout from all', () => {
    mockTokenService.deleteRefreshTokenForUser.mockImplementation(() =>
      of(loggedInUser),
    );

    service.logoutFromAll(loggedInUser).subscribe((result) => {
      expect(result).toStrictEqual(loggedInUser);
      expect(mockTokenService.deleteRefreshTokenForUser).toHaveBeenCalledWith(
        loggedInUser,
      );
    });
  });

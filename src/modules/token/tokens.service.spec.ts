...
describe('tokensService', () => {
  ...
  it('should generate refresh token', () => {
    ...
    mockRefreshTokenRepo.createRefreshToken.mockImplementation(() =>
      of(refreshToken),
    );
  ...
  it('should create access token from refresh token', () => {
    ...
    jest
      ...
      .mockImplementation(() => of('refreshToken'));
    service
  ...
  it('should delete all refresh token for user', () => {
    ...
    service.deleteRefreshTokenForUser(loggedInUser).subscribe((result) => {
      ...
      expect(mockRefreshTokenRepo.deleteTokensForUser).toHaveBeenCalledWith(
        loggedInUser,
      );
    });
  });
  ...
  it('should ge refresh token from payload for user', () => {
    mockRefreshTokenRepo.findTokenById.mockImplementation(() =>
      of(refreshToken),
    );
    service
      .getStoredTokenFromRefreshTokenPayload(refreshTokenPayload)
      .subscribe((result) => {
        expect(result).toStrictEqual(refreshToken);
        expect(mockRefreshTokenRepo.findTokenById).toHaveBeenCalledTimes(1);
        expect(mockRefreshTokenRepo.findTokenById).toHaveBeenCalledWith(
  ...
  it('should get user from refresh token payload', () => {
    service
      .getUserFromRefreshTokenPayload(refreshTokenPayload)
      .subscribe((result) => {
